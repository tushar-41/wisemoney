...
const BalanceSummary = (props) => {
  ...
  if(!props.balances) return null;
  const {oweDetails} = props.balances;
  console.log(props.balances.oweDetails);
        ...
        <div>
          ...
          <div className='space-y-3'>
            {oweDetails.youAreOwedBy.map((item) => (
                <Link key={item.userId} href={`/person/${item.userId}`} className='flex items-center justify-between hover:bg-muted p-2 rounded-md transition-colors'>
